"""Image-only RunPod worker for the bundled ComfyUI LTX 2.3 workflow."""
...
LOGGER = logging.getLogger("ltx23-worker")
...
MAX_INPUT_IMAGE_BYTES = int(os.getenv("MAX_INPUT_IMAGE_BYTES", str(20 * 1024 * 1024)))
...
def load_workflow(path: Path | None = None) -> dict[str, Any]:
    ...
    if not image_node or image_node.get("class_type") != "LoadImage":
        raise WorkerError(f"Image node {IMAGE_NODE_ID} is missing or is not LoadImage")
    if OUTPUT_NODE_ID not in workflow:
        raise WorkerError(f"Output node {OUTPUT_NODE_ID} is missing")
    ...
    if parsed.scheme in {"http", "https"}:
        try:
            with requests.get(
                ...
                stream=True,
                timeout=(15, 120),
                headers={"User-Agent": "runpod-ltx23-image-worker/1.0"},
            ) as response:
...
def queue_workflow(workflow: dict[str, Any], client_id: str) -> str:
    try:
        ...
        if not response.ok:
            ...
            raise WorkerError(
                f"ComfyUI workflow validation failed (HTTP {response.status_code}): {detail}"
            )
        result = response.json()
    except requests.RequestException as exc:
...
def handle_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(job_input) - {"image"}:
        return {"error": "Only input.image is supported"}
    ...
    try:
        ...
        uploaded_name = upload_input_image(
